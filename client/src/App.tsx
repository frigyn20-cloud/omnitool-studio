...
function SeoLandingPage() {
  ...
  return (
    <div className="quiet-grid min-h-screen">
      <Seo title={landing.title} description={landing.description} />
      <PublicNav dark={dark} setDark={setDark} />
      <main className="mx-auto grid max-w-7xl gap-4 px-4 pb-6 sm:px-5 lg:grid-cols-[1fr_20rem]">
        <section className="grid content-start gap-4">
          <section className="glass-panel rounded-[2rem] border hairline p-5 sm:p-7">
            <p className="mb-3 text-xs font-black uppercase tracking-[0.2em] text-primary">{language === "es" ? "Guía popular" : "Popular guide"}</p>
            <h1 className="max-w-3xl text-xl font-black tracking-[-0.04em]">{landing.title}</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">{landing.description}</p>
            <p className="mt-4 max-w-3xl text-sm leading-6 text-muted-foreground">{landing.useCase}</p>
          </section>
        ...
        <section className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {blogPosts.map((post) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className="glass-panel rounded-[2rem] border hairline p-5 transition hover:border-primary/50 hover:bg-accent">
              <p className="text-xs font-black uppercase tracking-[0.16em] text-primary">{post.readTime}</p>
              <h2 className="mt-3 text-lg font-black tracking-[-0.03em]">{post.title}</h2>
...
function BlogPostPage() {
  ...
  return (
    <div className="quiet-grid min-h-screen">
      ...
      <main className="mx-auto grid max-w-7xl gap-4 px-4 pb-6 sm:px-5 lg:grid-cols-[1fr_20rem]">
        <article className="glass-panel rounded-[2rem] border hairline p-5 sm:p-8">
          <p className="mb-3 text-xs font-black uppercase tracking-[0.2em] text-primary">{post.readTime}</p>
          <h1 className="max-w-3xl text-xl font-black tracking-[-0.04em]">{post.title}</h1>
